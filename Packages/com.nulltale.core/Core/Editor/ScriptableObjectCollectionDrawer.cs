...
namespace CoreLib
{
    [CustomPropertyDrawer(typeof(ScriptableObjectCollection<>), true)]
    [CustomPropertyDrawer(typeof(ScriptableObjectCollectionTypesAttribute), true)]
    public class ScriptableObjectCollectionDrawer : PropertyDrawer
    {
        ...
        private ReorderableList _getList(SerializedProperty property)
        {
            if (m_List == null)
            {
                ...
                if (attribute is ScriptableObjectCollectionTypesAttribute attr)
                ...
                m_List.onRemoveCallback = list =>
                {
                    ...
                    var doNotRemove = Event.current.modifiers == EventModifiers.Shift || isInner == false || hasValue == false 
                    ...
                    // remove link, then position
                    propertyList.DeleteArrayElementAtIndex(list.index);
                    if (hasValue)
                        propertyList.DeleteArrayElementAtIndex(list.index);
                };
                ...
                m_List.elementHeightCallback = index => propertyList.GetArrayElementAtIndex(index).GetObjectReferenceHeight();
            }
